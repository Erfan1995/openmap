...
const PublicUserManualMapData = ({ data, mapFilterData, formElementsName }, ref) => {
    ...
    const deleteDataset = async (record) => {
        setLoading(true)
        try {
            const res = await deleteMethod('mmdpublicusers/' + record.id)
    ...
    const updateState = async (record) => {
        ...
        setLoading(true);
        try {
            const res = await putMethod('mmdpublicusers/' + record.id, { is_approved: state });
            if (res) {
...
export default forwardRef(PublicUserManualMapData);