...
export async function getClientMapData(id, token = null, status = null) {
  try {
    const data = await fetchAPIGraphQl(`
  query
      { 
...
export async function getSurveyForms(customerId, token) {
  try {
    const data = await fetchAPIGraphQl(`
  query Posts($where: JSON)
      {
