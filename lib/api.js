...
export async function getIpLocation() {
  try {
    const response = await fetch("http://api.ipstack.com/45.126.253.106?access_key=a005e3b408ae19a6aba434b9c912e3fe");
...
export async function getWidgets(mdcId, mapId, token, progressbarType, mapConf) {
  try {
    const data = await fetchAPIGraphQl(`
      query
        {
          widgets(where: {map: ${mapId}}){
           id,
           text{
             title,
             color,
        ...
        ${progressbarType}(where:{${mapConf}:${mdcId}}){
          id,
          title,
          hover_text,
          is_active,
    ...
    query {
      maps(where: {id:${id}}) {
        ...
        mapsurveyconfs{
          ...
          selected_survey_properties,
          edited_survey_properties,
          listview_edited_survey_properties,
          listview_survey_properties,
          listview_edited_survey_properties,
          listview_survey_properties,
          progress_bar_default_style,
          selected_step,
        ...
        maps(where: {id:${id},mapId:"${mapId}"}) {
         ...
         logo{
          ...
          mapdatasetconfs{
            ...
            selected_dataset_properties,
            edited_dataset_properties,
            listview_dataset_properties,
            listview_edited_dataset_properties,
            icon{
              id,
              icon{
                id,
