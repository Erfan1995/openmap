...
import { getOneMap, getMMDPublicUser, getMMDCustomersClients, putMethodPublicUser, checkPublicUsersMapBased, getPublicUsers, postMethodPublicUser, getMapVisits } from "./api";
        ...
        if (data) {
            ...
            data?.surveys?.map((obj) => {
                ...
                [...obj.mmdpublicusers].map((item) => {
                    if (!item.is_approved && item?.public_user?.publicAddress === publicAddress) {
                        manualArray.push(
                            {
                                type: "Feature",
                                geometry: item.geometry,
...
export const generateListViewData = (manualMapData, survey) => {
    let listviewData = [];
    manualMapData.map((data) => {
        ...
        listviewData.push({
            'surveyData': eachSurveyResult, 'pubDate': data.pubDate, 'surveyInfo': surveyInfo,
            'progressbar': data.mapSurveyConf.survey_progressbars, 'latlng': data.properties.geolocation
        });
    })
...
export const publicUserOperation = async (publicAddress, mapData) => {
    try {
        ...
        if ((res?.length > 0)) {
            ...
            const response = await postMethodPublicUser('public-users', { publicAddress: publicAddress, maps: mapData.id });
            if (response) {
                Router.push({
                    pathname: '/client/map',
