...
export const getMapData = async (id) => {
    ...
    try {
        if (id) {
            const data = await getOneMap({ id: id }, null, false);
...
export const getSpecifictPopup = (properties, type) => {
    switch (type) {
      case 'dark-mode': return `<div class='dark-mode'>${generateData(properties,type)}</div>`
      case 'white-mode': return `<div class='white-mode'>${generateData(properties,type)}</div>`
      case 'color-mode': return `<div class='color-mode '>${generateData(properties,type)}</div>`
      default :  return `<div class='white-mode'>${generateData(properties,type)}</div>`
    }
  }
